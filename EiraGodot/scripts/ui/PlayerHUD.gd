...
func refresh() -> void:
	var location := DataLoader.get_location(GameState.current_location_id)
	var location_name := str(location.get("name", GameState.current_location_id))
	var hero_name := str(GameState.current_character.get("name", GameState.current_character_id))
