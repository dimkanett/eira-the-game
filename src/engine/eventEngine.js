...
import { adjustDecayValues, getWorldDecayStage } from "./decayEngine.js";
...
export function getEventTextForState(event, gameState) {
  if (!event) return "";
  const stage = getWorldDecayStage(gameState.worldDecay || 0);
  return event.eventsByDecay?.[stage] || event.text;
}

function prepareEventForState(event, gameState) {
  if (!event) return null;
  return { ...event, text: getEventTextForState(event, gameState) };
}

export function startEvent(gameState, eventId) {
  const state = cloneState(gameState);
  const event = allEvents().find((item) => item.id === eventId);
  state.activeEvent = prepareEventForState(event, state);
...
export function applyEffects(gameState, effects = []) {
  ...
  for (const effect of effects) {
    ...
    if (effect.type === "world_decay") state = adjustDecayValues(state, { worldDecay: effect.value }, { writeMessage: false });
    if (effect.type === "personal_decay") state = adjustDecayValues(state, { personalDecay: effect.value }, { writeMessage: false });
    if (effect.type === "sin") state = adjustDecayValues(state, { sin: effect.value }, { writeMessage: false });
