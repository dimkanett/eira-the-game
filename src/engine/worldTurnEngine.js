...
import { processWorldDecayTurn } from "./decayEngine.js";
...
export function nextDay(gameState) {
  ...
  const decayTurn = processWorldDecayTurn(state);
  state = decayTurn.state;

  if (state.travel?.active) return advanceTravelDay(state);

  state.activeMessage = `День ${state.hero.day}. NPC сделали скрытые ходы. Их маршруты неизвестны. ${decayTurn.message}`;
