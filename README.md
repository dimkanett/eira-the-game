...
- глобальная карта: `public/assets/maps/world_map.png` (должна совпадать с координатной системой `WORLD_MAP_SIZE` в `src/data/worldNodes.js`);
- изображения локаций: `public/assets/locations/`;
- изображения городов: `public/assets/cities/`;
- персонажи: `public/assets/characters/` (`lorien_elf_default.png`, `lorien_elf_wounded.png`, `lorien_elf_badly_wounded.png` и варианты ранений);
...
- Глобальная карта с точками, связями и туманом войны; карта, маршруты и точки рисуются в единой фиксированной пиксельной системе координат, чтобы точки не съезжали при изменении размера окна.
...
- Инвентарь, журнал, репутация, книга лора и подробный портрет персонажа спрятаны в модальные окна.
